...
# This is a modified version of https://github.com/Syzik/DockerRegistryGrabber that allows you to use JWTs for auth
# If you have a JWT simply put it in the password fiels and leave username unset!
...
def manageArgs():
    ...
    auth.add_argument('-P', dest='password', type=str, default="", help='Password or JWT')
...
def tryReq(url, username=None,password=None):
    try:
        if username and password:
            ...
            r = req.get(url,verify=False, headers={"Authorization": "Bearer " + password})
            r.raise_for_status()
        else:
        ...
        console.print(f"Response auth header {r.headers}")
        sys.exit(1)
